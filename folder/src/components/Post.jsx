 ...
 const user = useSelector((state)=> state.user.user);
  ...
  const handlelikeanddislike = async() =>{
    try {
      ...
      console.log(token)
      const response = await axios.put(`https://instagramclone-sxrf.onrender.com/api/posts/${post._id}/like`,{},
       {  
         headers: {
  ...
  const handlefollowandunfollow = async() =>{
    try {
      ...
      const token = localStorage.getItem("token"); // Ensure token is stored
      console.log(token)
      const response = await axios.put(`https://instagramclone-sxrf.onrender.com/api/posts/follow/${userToFollowId}`,{},
       {  
        headers: {
...
const handleDelete = async () => {
  try {
    const token = localStorage.getItem("token"); // Ensure token is stored
    console.log(token)
    const response = await axios.delete(`https://instagramclone-sxrf.onrender.com/api/posts/delete/${post._id}`, {
      headers: {
...
const handlecomment = async(e) =>{
  ...
  try {
    const token = localStorage.getItem("token"); // Ensure token is stored
    console.log(token)
    const response = await axios.post(`https://instagramclone-sxrf.onrender.com/api/posts/comment/${post._id}`,{text},{
      headers: {
